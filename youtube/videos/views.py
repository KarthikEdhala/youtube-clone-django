...
def video_upload(request):
    ...
    if form.is_valid():
        ...
        custom_thumbnail=request.POST.get("tumbnail_data","")
        print(custom_thumbnail)
        try:
            result=upload_video(
...
def delete_video(request,video_id):
    ...
    try:
        ...
        pass
    video.delete()
    return JsonResponse({"success":True,"message":"Video deleted sucessfully"})
...
def video_vote(request,video_id):
    ...
    if existing_vote:
        ...
        existing_vote=VideoLike.objects.create(user=request.user,video=video,value=value)
        if VideoLike.LIKE==value:
            video.likes+=1
        else:
            video.dislikes+=1
        user_vote=value
    video.save(fields=['likes','dislikes'])
